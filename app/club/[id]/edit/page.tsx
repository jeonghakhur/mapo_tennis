...
export default function ClubEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { clubs, updateClubById } = useClubs();
  ...
  useEffect(() => {
    const found = clubs.find((c) => c._id === id);
    if (found) {
      setClub(found);
      setName(found.name || '');
      setDescription(found.description || '');
      setWorkDays(found.workDays || '');
      setLocation(found.location || '');
      setIsPublic(found.isPublic ?? true);
      setContact(found.contact || '');
      if (found.image?.asset?._ref) {
        setPreview(urlFor(found.image).url());
      }
    }
  }, [clubs, id]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await updateClubById(id, updateData, image ?? undefined);
