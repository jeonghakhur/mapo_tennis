...
export default function WelcomePage() {
  ...
  // SWR로 회원 존재 여부 확인 (fetcher는 SWRConfigContext에서 전역 제공)
  const { data, isLoading } = useSWR(email ? `/api/user/?email=${email}` : null);

  useEffect(() => {
    setAlreadyRegistered(!!data?.user);
  }, [data?.user]);
  ...
  const isHydrating = typeof window === 'undefined' || isLoading || data === undefined || !data;

  return (
    <Container>
      {isHydrating || alreadyRegistered ? (
        <AlreadyRegisteredDialog open={!!alreadyRegistered} />
      ) : (
        <>
          {!session?.user ? (
            <Box mb="4">
